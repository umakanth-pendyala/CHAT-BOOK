...
router.get('/requestHistory', async (req, res) => {
    ...
    sendingInformation.pageTitle = "Friend Requests By You"
    ...
    if (req.session.userInformation.userIsAuthenticated) {
        ...
        sendingInformation.titleSheet = await getrequestHistoryTileSheet(userDocument);
...
const getrequestHistoryTileSheet = async (userDocument) => {
    var sendingInformation = [];
    const userFriendRequests = userDocument.friendRequestsByYou;
